	  ...
	  closeDialog() {
  	  ...
  	  updatephone(formName){
  		  ...
  		  if(_this.changePhoneForm.phone==""){
  			  ...
  			  _this.changePhoneForm.id = _this.doctor.id
  			  axios.put('http://localhost:8181/doctor/updatephone',_this.changePhoneForm).then(function(resp){
  			  	if(resp.data.code == 0){
  			  		alert('修改成功！请重新登录！')
	   ...
	   updatepassword(formName){
		  ...
		  if(_this.changePasswordForm.oldphone=="" || _this.changePasswordForm.newphone==""){
			  ...
			  _this.changePasswordForm.id = _this.doctor.id
			  axios.put('http://localhost:8181/doctor/updatepassword',_this.changePasswordForm).then(function(resp){
				if(resp.data.code == 0){
					alert('修改成功！请重新登录！')
				_this.$router.replace({path:'/login'})
				}else{
		...
		createnewkey(){
			const _this = this
			axios.post('http://localhost:8181/doctor/createnewkey/'+ _this.doctor.id)
			.then(function(resp){
				if (resp.data.code === 0) {
